...
class ImgPair(torchvision.datasets.VisionDataset):
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        selected_img_path_l = random.choices(img_path_l, k=2)
        img_1 = pil.Image.open(selected_img_path_l[0])
        img_2 = pil.Image.open(selected_img_path_l[1])
        view_1 = self.transforms(img_1)
        view_2 = self.transforms(img_2)
        return ((view_1, view_2), 0)
    
    def __len__(self) -> int:
        return len(self.img_by_folder_l)
